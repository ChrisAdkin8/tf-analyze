...
export const ENGINE_MAX_BUFFER = 50 * 1024 * 1024;

export interface EngineResult {
  /** Underlying spawn error, if any (includes the exit code under `.code`). */
  err: (cp.ExecException & { code?: number }) | null;
  stdout: string;
  stderr: string;
  /** Pretty cmdLine string suitable for displaying in error HTML. */
  cmdLine: string;
  /** True when the helper killed the process for exceeding the timeout. */
  timedOut: boolean;
}
...
export function runEngine(scriptPath: string, args: string[], cb: EngineCallback): void {
  ...
  let settled = false;
  const finish = (result: EngineResult): void => {
  ...
  const proc = cp.execFile(
    ...
    (err, stdout, stderr) => {
      finish({
        err: err as (cp.ExecException & { code?: number }) | null,
        stdout: stdout || '',
        stderr: stderr || '',
        cmdLine,
        timedOut: false,
  ...
  const timer = setTimeout(() => {
    ...
    finish({
      err: { name: 'TimeoutError', message: `engine exceeded ${ENGINE_TIMEOUT_MS / 1000}s and was cancelled`, code: 124 } as cp.ExecException & { code?: number },
      stdout: '',
      stderr: `tf-analyze: engine exceeded ${ENGINE_TIMEOUT_MS / 1000}s and was cancelled. Re-run from a smaller workspace or open the Output panel for the engine command.`,
      cmdLine,
      timedOut: true,
    });
  }, ENGINE_TIMEOUT_MS);
 ...
 */
export function classifyEngineResult(result: EngineResult): {
  ok: boolean;
