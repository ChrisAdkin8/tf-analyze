...
import { BlastRadiusProvider } from "./blastRadiusView";
import { BlastRadiusCodeLensProvider } from "./blastRadiusLens";
...
interface Finding {
  ...
  mitre?: string[];
  resource?: string;
  // R30.18 — populated when --attack-graph ran; downstream count for
  // this finding's resource. The status bar surfaces a high-blast
  // count, the diagnostic message appends `🌊 blast: N`, the tree
  // view sorts by it, and the LSP severity uplift uses it.
  blast_radius?: number;
}

// R30.17 — top-N high-blast resources, sorted by `blast_radius` desc.
// Independent of `findings`: a resource can be high-blast even if it
// has no findings on it — that's the whole point of blast radius
// being orthogonal to urgency.
...
function _resolveFindingPath(file: string, target: string): string {
...
const FILTERABLE_SEVERITIES = ["CRITICAL", "HIGH", "MEDIUM", "LOW"] as const;

class FilterGroupItem extends vscode.TreeItem {
  constructor(
    public readonly counts: Map<string, number>,
    public readonly hidden: Set<string>,
  ) {
    const totalSev = FILTERABLE_SEVERITIES.filter((s) => (counts.get(s) ?? 0) > 0).length;
    const visibleSev = FILTERABLE_SEVERITIES.filter(
      (s) => !hidden.has(s) && (counts.get(s) ?? 0) > 0,
    ).length;
    const suffix =
      hidden.size === 0 ? "showing all" : `${visibleSev} of ${totalSev}`;
...
class SeverityFilterItem extends vscode.TreeItem {
  constructor(public readonly severity: string, count: number, visible: boolean) {
...
class FindingsProvider implements vscode.TreeDataProvider<TreeNode> {
  ...
  private hiddenSeverities = new Set<string>();
  ...
  setHiddenSeverities(severities: Iterable<string>): void {
    this.hiddenSeverities = new Set([...severities].map((s) => s.toUpperCase()));
    this._onDidChangeTreeData.fire();
  }

  getHiddenSeverities(): string[] {
    return [...this.hiddenSeverities];
  }

  toggleSeverity(sev: string): boolean {
  ...
  private visibleFindings(): Finding[] {
    if (this.hiddenSeverities.size === 0) return this.findings;
  ...
  private severityCounts(): Map<string, number> {
  ...
  getChildren(element?: TreeNode): TreeNode[] {
    ...
    if (!element) {
      ...
      const filterGroup = new FilterGroupItem(counts, this.hiddenSeverities) as unknown as TreeNode;
      ...
      if (visible.length === 0) {
        ...
        return [filterGroup, empty as unknown as TreeNode];
      }
    ...
    if (element instanceof FilterGroupItem) {
      const counts = this.severityCounts();
      return FILTERABLE_SEVERITIES.map(
        (sev) =>
          new SeverityFilterItem(sev, counts.get(sev) ?? 0, !this.hiddenSeverities.has(sev)),
...
function upliftSeverityByBlast(
  base: vscode.DiagnosticSeverity,
  blast: number,
...
// Audit item 13 — runScan previously took 9 positional params; every
// call-site had to retype the order. A `ScanContext` makes the
// dependency surface explicit and removes the foot-gun where two
// status-bar items in the middle of the list could be transposed.
interface ScanContext {
  diagnosticCollection: vscode.DiagnosticCollection;
  provider: FindingsProvider;
  findingsMap: Map<string, Finding[]>;
  statusBar: vscode.StatusBarItem;
  blastStatusBar: vscode.StatusBarItem;
  blastProvider: BlastRadiusProvider;
  blastLensProvider: BlastRadiusCodeLensProvider;
...
async function runScan(ctx: ScanContext): Promise<void> {
  const {
  ...
  if (_scanInFlight) {
    outputChannel.appendLine(
      "[tf-analyze] Scan already in flight — skipping duplicate request.",
    );
    return;
  }
  ...
  try {
    const result = await new Promise<string>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        reject(new Error(
          `tf-analyze: scan exceeded ${SCAN_TIMEOUT_MS / 1000}s and was cancelled. ` +
    ...
    if (highBlastCount > 0) {
      ...
      blastStatusBar.color = new vscode.ThemeColor(
        highBlastCount >= 3 ? "errorForeground" : "problemsWarningIcon.foreground",
      );
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const persistedHidden = context.workspaceState.get<string[]>(FILTER_STATE_KEY, []);
  ...
  treeView.onDidChangeCheckboxState((e) => {
    ...
    for (const [item, state] of e.items) {
      if (!(item instanceof SeverityFilterItem)) continue;
  ...
  const blastProvider = new BlastRadiusProvider();
  const blastTreeView = vscode.window.createTreeView("tfAnalyzeBlastRadius", {
    treeDataProvider: blastProvider,
    showCollapseAll: true,
  });
  const blastLensProvider = new BlastRadiusCodeLensProvider();
  const blastLensRegistration = vscode.languages.registerCodeLensProvider(
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("tf-analyze.runScan", () => runScan(scanCtx)),

    ...SEVERITIES.map((sev) =>
      vscode.commands.registerCommand(`tf-analyze.toggle${sev.charAt(0)}${sev.slice(1).toLowerCase()}`, () => {
