 ...
 */
export class RemediationPanel {
  static currentPanel: RemediationPanel | undefined;
  ...
  static createOrShow(context: vscode.ExtensionContext): void {
    ...
    const panel = vscode.window.createWebviewPanel(
      'tfAnalyzeRemediation',
      'tf-analyze: Remediate',
      col,
  ...
  private _splitDiffFromSummary(out: string): { diffOnly: string; hasFix: boolean } {
    const lines = out.split('\n');
    const cut: string[] = [];
    let hasFix = false;
    for (const line of lines) {
      if (line.startsWith('# tf-analyze:')) break;
      if (line.startsWith('--- ')) hasFix = true;
      cut.push(line);
    }
  ...
  private async _apply(): Promise<void> {
    const choice = await vscode.window.showWarningMessage(
      'Apply tf-analyze fixes? This rewrites .tf files on disk. Originals will be saved as <file>.bak alongside each patched file.',
      { modal: true },
      'Apply'
    );
  ...
  private _renderDiff(diff: string, cmdLine: string): string {
    return `<!DOCTYPE html>
...
<script>
  ...
  private _renderApplied(diff: string): string {
    return `<!DOCTYPE html>
<html><head><meta charset="UTF-8"><style>
  ${this._sharedCss()}
  #diff { font-family: ui-monospace, Menlo, monospace; font-size: 12px; line-height: 1.5; padding: 16px 24px; white-space: pre; overflow: auto; }
...
<script>
  ...
  private _renderEmpty(rawOutput: string, cmdLine: string): string {
    return `<!DOCTYPE html>
...
<script>
  ...
  private _highlightDiff(diff: string): string {
    const escaped = this._escape(diff);
    return escaped
      .split('\n')
      .map(line => {
        if (line.startsWith('--- ') || line.startsWith('+++ ')) return `<span class="file-hdr">${line}</span>`;
        if (line.startsWith('@@')) return `<span class="hunk-hdr">${line}</span>`;
  ...
  private _loading(msg: string): string {
