...
class RuleExplainerPanel {
    ...
    _render(text) {
        ...
        const escaped = this._escape(text)
            ...
            .replace(/^# CIS: (.+)$/gm, (_m, items) => `<div class="taxon"><span class="taxon-label">CIS</span>${items.split(',').map(s => `<span class="chip chip-cis">${s.trim()}</span>`).join('')}</div>`)
            .replace(/^# MITRE ATT&amp;CK: (.+)$/gm, (_m, items) => `<div class="taxon"><span class="taxon-label">MITRE ATT&amp;CK</span>${items.split(',').map(s => `<a class="chip chip-mitre" href="https://attack.mitre.org/techniques/${s.trim().replace('.', '/')}/" target="_blank" rel="noopener">${s.trim()}</a>`).join('')}</div>`)
            .replace(/^# CWE: (.+)$/gm, (_m, items) => `<div class="taxon"><span class="taxon-label">CWE</span>${items.split(',').map(s => {
            const id = s.trim();
            ...
            .replace(/^# MITRE D3FEND: (.+)$/gm, (_m, items) => `<div class="taxon"><span class="taxon-label">D3FEND</span>${items.split(',').map(s => `<a class="chip chip-d3fend" href="https://d3fend.mitre.org/technique/${s.trim()}/" target="_blank" rel="noopener">${s.trim()}</a>`).join('')}</div>`)
            ...
            .replace(/\b((?:SEC|ROB|STK|OPS|MOD|COST|INT|CI|STYLE|CUSTOM|DRY)-[A-Z0-9-]+)\b/g, (_m, id) => id === this._ruleId ? id : `<a href="${(0, urls_1.ruleDocsUrl)(id)}">${id}</a>`);
        return `<!DOCTYPE html>
