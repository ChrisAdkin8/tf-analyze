...
- [~] **P1 · L — `detect.py` modularization.** Started at 8,400+ LoC monolith; **five seams shipped** so far (`_mitre.py`, `_versions.py`, `_scoring.py`, `_hcl.py`, `_catalog.py` — landed across R30.0.5, R30.0.6, R30.0.7, R30.0.8). detect.py now ~7,670 LoC (8,441 → 7,669, **−772** over five sub-rounds); the five extracted modules total ~1,184 LoC of pure-function helpers + immutable constants, all with re-export shims so callers don't migrate. The 634 passing tests are the safety net — refactor is incremental and proven. Remaining splits to do:
