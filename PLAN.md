...
- [~] **P1 · L — `detect.py` modularization.** Started at 8,400+ LoC monolith; **six seams shipped** so far (`_mitre.py`, `_versions.py`, `_scoring.py`, `_hcl.py`, `_catalog.py`, `_attack_graph.py` — landed across R30.0.5, R30.0.6, R30.0.7, R30.0.8, R30.0.9). detect.py now ~6,985 LoC (8,441 → 6,985, **−1,456 / 17.2%** over six sub-rounds); the six extracted modules total ~1,996 LoC of pure-function helpers + immutable constants, all with re-export shims so callers don't migrate. The 639 passing tests are the safety net — refactor is incremental and proven. Remaining splits to do:
