...
State of the world at time of writing (2026-05-10): 217 rules · 644 pytest cases + 24 `node:test` cases · extension v0.1.38 · `action.yml` posts `--format pr-summary` blocks (R28.1 actually wired, was claim-only before R30.0.2) · `v0.1.0` tagged · per-rule docs site live with JSON-LD + family backlinks + 4-verb `vscode://` URI handler + OWASP IaC + CWE + D3FEND references · Module Reuse Advisor with ROI signal · status-bar grade badge · **ten surfaces** (all with R29 framework parity; MCP server hardened against agent-side abuse; GitHub Pages site has dedicated pages for all ten surfaces) + four compliance frameworks (CIS, PCI-DSS, SOC 2, OWASP IaC) + three vulnerability/threat taxonomies (MITRE ATT&CK pinned to v17 with 69% coverage, CWE 53%, MITRE D3FEND 40%) — SARIF v2.1 emits proper structured taxonomies + relationships (R30.0.5), CI ATT&CK-drift gate locks the technique table. detect.py modularisation underway: **seven seams shipped** (`_mitre.py`, `_versions.py`, `_scoring.py`, `_hcl.py`, `_catalog.py`, `_attack_graph.py`, `_output.py`) reducing detect.py 8,441 → 5,528 LoC (**−2,913 / 34.5%**) across R30.0.5–R30.0.11; monolith below 6k LoC for the first time since R12.
...
- [~] **P1 · L — `detect.py` modularization.** Started at 8,400+ LoC monolith; **seven seams shipped** so far (`_mitre.py`, `_versions.py`, `_scoring.py`, `_hcl.py`, `_catalog.py`, `_attack_graph.py`, `_output.py` — landed across R30.0.5–R30.0.11). detect.py now ~5,528 LoC (8,441 → 5,528, **−2,913 / 34.5%** over seven sub-rounds); the seven extracted modules total ~3,615 LoC of pure-function helpers + immutable constants + data tables, all with re-export shims so callers don't migrate. The 644 passing tests are the safety net — refactor is incremental and proven. **Monolith below 6k LoC for the first time since Round 12.** Remaining splits to do:
