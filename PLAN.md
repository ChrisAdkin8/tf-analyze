...
- [~] **P1 · L — `detect.py` modularization.** Started at 8,400+ LoC monolith; **three seams shipped** so far (`_mitre.py`, `_versions.py`, `_scoring.py` — landed across R30.0.5 and R30.0.6). detect.py now ~8,200 LoC; the three extracted modules total ~420 LoC of pure-function helpers, all with re-export shims so callers don't migrate. The 624 passing tests are the safety net — refactor is incremental and proven. Remaining splits to do:
