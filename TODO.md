...
State at last sync (2026-05-11): **236 rules · 824 tests** + 62 node:test · ext v0.1.46 · R30.9 follow-up audit shipped (20 of 24 follow-up findings closed; shared `engineRunner.ts` closes panel boilerplate + missing timeouts + leaked subscriptions in one seam; `bundle-engine.js` globs `scripts/_*.py`; `pytest-timeout=300` on every CI matrix slot; `_hcl.block_arg_value` backslash-aware; `/explain?file=...` URI handler gated to workspace root; detect.py `--output` file-handle leak closed via atexit) · `detect.py` 4,378 LoC after R30.19 Sessions H–O (-48.1% since modularisation began) · 🌊 Blast Radius tree view + CodeLens + status-bar chip + diagnostic uplift (R30.18; panel wiring fixed in R30.7 / v0.1.44 — `buildArgs()` now passes `--attack-graph`) · `action.yml` posts `--format pr-summary` blocks · `v0.1.0` tagged · per-rule docs site live · Module Reuse Advisor (ROI) · `vscode://` URI handler · status-bar score+grade badge · badge service · MCP server (hardened, R30.0) · Terraform provider · HCP Run Task · GitHub Action · **public web scanner at `tfanalyze.com/scan/<owner>/<repo>` (R30.14)** · **OWASP IaC + NIST CSF + NIST 800-53 + CSA CCM + SLSA + MITRE ATT&CK v17 (69%) + CWE (53%) + D3FEND (40%)** all tagged in catalogue · **13 active `--compliance-framework` modes — all 9 R30.1 modes now surface real data after R30.9 bulk tag** · `--rank-by exploitability` (CISA KEV + FIRST.org EPSS, R30.2) · `--explain-score` top-5 ranking (R30.8) · workflow-YAML walker activates SEC-CICD-001/002/003 (R30.6) · **`--mode drift --state-json` for oncalls (R30.12)** · **`--pdf-output` for CISOs (R30.13)** · **`--apply-fixes × --baseline` + `--mode diff × --baseline` composition (R30.11)** · **`fix_hcl_minimal:` catalogue field (R30.10)** · SARIF v2.1 structured taxonomies + relationships (R30.0.5) · detect.py modularisation: **9 seams shipped** — `_lsp.py` (R30.7); detect.py 8,441 → ~5,170 LoC (−3,271 / **38.7%**) across eight sub-rounds.
