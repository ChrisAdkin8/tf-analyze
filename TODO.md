...
State at last sync (2026-05-12): **238 rules · 872 tests** + 62 node:test · ext v0.1.53 · R30.15 detector topic-module split shipped (51 inline handlers extracted into 5 topic modules under `scripts/_handlers_*.py` — 12 generic + 9 security + 14 robustness + 7 modules + 9 infra; detect.py 4377 → 3052 LoC; back-imports resolved via bottom-of-file import order + `sys.modules.setdefault("detect", …)` __main__ alias). R30.14 detector dispatch table shipped (51 elif kind branches → 51 registered handlers across 7 incremental commits; detect_in_file + detect_corpus collapsed to 25-LoC dispatch loops; total `elif kind ==` arms in detect.py: 51 → 0). After R30.13 (brace_walk) + R30.14 (dispatch table) + R30.15 (topic-module split), the three largest structural items every prior audit round flagged are closed. — one quote-aware bracket-depth helper in `_hcl.py` replaces 13 duplicated walkers across `detect.py` + `_apply_fixes.py`. Edge-case correctness improvements at 12 of 13 migrated sites (quoted braces in IAM ARNs, Helm values, jsonencode bodies no longer corrupt depth tracking). Audit cycle from rounds 1-5 now structurally closed (15 of 17 round-5 findings closed; GitHub Actions templating injection closed in action.yml; `_cmd_explain` path-traversal closed; all 5 `_diff.py` git subprocess calls now timeout-bounded; run-task `Popen` + `kill` on timeout; `_apply_fixes` brace walker quote-aware; **95 audit findings closed across 5 cumulative rounds**) (17 of 21 round-4 findings closed; HTML XSS class closed in both `_output.py` and `_attack_graph.py` standalone HTML to match R30.8's TypeScript fix; `_mermaid_id` collision-resistant via hash suffix; `cross_module` matches `module_unused`'s OSError handling; SYN-SCAN-FAILED uses valid `section`; gen_rule_docs.py uses N+1-length code fences for regex content) (20 of 22 round-3 findings closed; `_hcl.strip_hcl_context` now preserves newlines inside block comments — fixes the line-counting bug the R30.9 workaround tried but failed to address; `_lsp.py` rejects `*args` callables; `_output.py` urgency rank consolidated to one constant; tests/helpers.run_detect + self_test.py + GitHub Action + terraform-provider compliance + demo/app + badge-service all surface engine crashes with stderr instead of swallowing them) · `detect.py` 4,378 LoC after R30.19 Sessions H–O (-48.1% since modularisation began) · 🌊 Blast Radius tree view + CodeLens + status-bar chip + diagnostic uplift (R30.18; panel wiring fixed in R30.7 / v0.1.44 — `buildArgs()` now passes `--attack-graph`) · `action.yml` posts `--format pr-summary` blocks · `v0.1.0` tagged · per-rule docs site live · Module Reuse Advisor (ROI) · `vscode://` URI handler · status-bar score+grade badge · badge service · MCP server (hardened, R30.0) · Terraform provider · HCP Run Task · GitHub Action · **public web scanner at `tfanalyze.com/scan/<owner>/<repo>` (R30.14)** · **OWASP IaC + NIST CSF + NIST 800-53 + CSA CCM + SLSA + MITRE ATT&CK v17 (69%) + CWE (53%) + D3FEND (40%)** all tagged in catalogue · **14 `--compliance-framework` CLI modes — 11 with tagged rule data (owasp_cicd / owasp_llm / owasp_asvs ship as flags but have 0 tagged rules; treat as stubs awaiting bulk-tag follow-up)** · `--rank-by exploitability` (CISA KEV + FIRST.org EPSS, R30.2) · `--explain-score` top-5 ranking (R30.8) · workflow-YAML walker activates SEC-CICD-001/002/003 (R30.6) · **`--mode drift --state-json` for oncalls (R30.12)** · **`--pdf-output` for CISOs (R30.13)** · **`--apply-fixes × --baseline` + `--mode diff × --baseline` composition (R30.11)** · **`fix_hcl_minimal:` catalogue field (R30.10)** · SARIF v2.1 structured taxonomies + relationships (R30.0.5) · detect.py modularisation: **9 seams shipped** — `_lsp.py` (R30.7); detect.py 8,441 → ~5,170 LoC (−3,271 / **38.7%**) across eight sub-rounds.
...
- [x] **P1 · L** **R31.4** Trend dashboard at `tfanalyze.com/trend/<owner>/<repo>` — sparkline + biggest-jump annotation + per-commit velocity table + OG card. 6 tests in `tests/test_public_scanner.py::TestTrendDashboard`. *(Round 31, 2026-05-12)*
- [x] **P1 · M** **R31.2** Auto-remediation PR bot — `integrations/github-action-bot.yml` runs on schedule, applies `fix_disruption: none` fixes via new engine flag `--apply-fixes-max-disruption`, opens one PR per repo grouped by rule family. 16 tests in `tests/test_github_action_bot.py` (9 workflow drift gates + 7 PR-body renderer unit tests). *(Round 31, 2026-05-12)*
