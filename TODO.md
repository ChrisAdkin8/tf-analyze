...
State at last sync (2026-05-11): **232 rules · 658 tests** + 24 node:test · ext v0.1.39 · `action.yml` posts `--format pr-summary` blocks · `v0.1.0` tagged · per-rule docs site live · Module Reuse Advisor (ROI) · `vscode://` URI handler · status-bar score+grade badge · badge service · MCP server (hardened, R30.0) · Terraform provider · HCP Run Task · GitHub Action · **OWASP IaC + NIST CSF + NIST 800-53 + CSA CCM + SLSA + MITRE ATT&CK v17 (69%) + CWE (53%) + D3FEND (40%)** all tagged in catalogue · **13 active `--compliance-framework` modes** (cis, pci_dss, soc2, owasp_iac + new R30.1: nist_csf, nist_800_53, csa_ccm, slsa, owasp_top10, owasp_api, owasp_cicd, owasp_llm, owasp_k8s, owasp_asvs) · SARIF v2.1 structured taxonomies + relationships (R30.0.5) · detect.py modularisation: **8 seams shipped** — R30.0.12; detect.py 8,441 → 5,116 LoC (−3,325 / **39.4%**) across seven sub-rounds.
