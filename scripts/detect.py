...
# `_URGENCY_TIERS` stays in detect.py — it's the ordered list (LOW <
# MEDIUM < HIGH < CRITICAL) used by grade-threshold comparisons, not a
# validation set. The validation sets (`_VALID_SECTIONS`,
# `_VALID_URGENCIES`, `_VALID_BLAST_RADIUS`, `_VALID_STATUS`,
# `_REQUIRED_FIELDS`) and `validate_catalog_entry` are re-imported
# from `_catalog.py` (fifth seam).
